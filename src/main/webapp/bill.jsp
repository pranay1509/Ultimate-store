...
try
{
	...
	ResultSet rs2= st.executeQuery("select *from users inner join cart where cart.email='"+email+"' and cart.status like '%bill%'");
	while(rs2.next())
	{
		
%>
<h3>ULTIMATE STORE</h3>
<hr>
...
<div class="right-div-right"><h3>Hostel:  <%=rs2.getString(14) %></h3></div> 
<div class="right-div"><h3>RoomNo:  <%=rs2.getString(15) %></h3></div> 
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1 = st.executeQuery("select *from cart inner join product where cart.product_id=product.id and cart.email='"+email+"' and cart.status like '%bill%'");
 while(rs1.next())
 {
	 sno=sno+1;
